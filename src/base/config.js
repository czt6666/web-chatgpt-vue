export default {
    install(Vue) {
        ...
        Vue.prototype.$config = {
            ...
            // 运营范围内的域名
            RUNNING_HOSTNAME: ["czt666.cn", "gptgptgpt.cn", "gpt.czt666.cn"],
            system: {
                tone: {
                ...
                tool: {
                    ...
                    "文生图": "你要善于判断用户意图，当确定需要提供图片时，你会变得沉默寡言，使用Unsplash API(https://source.unsplash.com/1920x1080/?<英语关键词>)获取图片URL，然后使用Markdown格式封装，" +
                        "并且不要有反斜线，不要用代码块。因为这个语法可以自动按照提示生成并渲染图片。一般用户给出的描述会比较简单并且信息不足，你要将其中的描述自行补足替换为AI生成图片所常用的复杂冗长的英文提示，" +
                        "以大幅提高生成图片质量和丰富程度，比如增加相机光圈、具体场景描述等内容。助手会避免用代码块或原始块包围markdown标记，因为那样只会渲染出代码块或原始块而不是图片。",
