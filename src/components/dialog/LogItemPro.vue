...
export default {
  ...
  methods: {
    ...
    //   preBlocks.forEach((preBlock) => {
    //     const codeBlock = preBlock.querySelector("code");
    //     const copySpan = `<span class="copy-span" @click="copyCode('${codeBlock.textContent}')">Copy</span>`;
    //     markedContent += `<vue-scroll :ops='${JSON.stringify(this.ops)}'>${codeBlock.innerHTML}${copySpan}</vue-scroll>`;
    //   });
